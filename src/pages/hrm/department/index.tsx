...
import { getDepartment,deleteDepartment} from './services'
...
const Department = () => {
  ...
  const [modalData,setModalData] = useState<DepartmentTypeAPI.Department>({})
  ...
  useEffect(()=>{
    getDepartment({
      ...
      setTreeDepartment(FlatToNested({},value.data,null)?.children[0]?.children)
    },error=>{
      console.log(error)
      setTreeDepartment([])
    })
      ...
      <Card>
        ...
        <Row gutter={24}>
          <Col xs={24} sm={24} md={10} lg={6} xl={6}>
            <LeftTree
              treeDatas={treeDepartment}
              onSelect={onSelect}
            />
          </Col>
          <Col xs={24} sm={24} md={14} lg={18} xl={18}>
            <RightTable
              tableScrollWidth={1280}
